                ...
                <tbody name="modalTable-body">
                    <tr class="even pointer" id="data_row" name="data_row" role="row">
                        ...
                        {foreach $row as $value}
                            {if $value@key == category || $value@key == group}
                            ...
                            {else}
                                <td id="{$value@key}" class=" " name="{$value@key}" {if $value@key == id} style="width:7%; margin-right:5px;"{/if}>
                                    <input id="{$value@key}" class="table_td-input" name="table_input-field" value="{$value}" {if $value@key == id || $value@key == author} readonly="readonly" {/if}/>
                                </td>
                            {/if}
                        {/foreach}
