...
function retrieve_all_reports(){
    $query = selectQuery("report", "", "date DESC");
...
function get_report($id){
    $report = selectRecord("report", "id = $id");
...
function set_archive($id, $archive){
    ...
    if($archive)
        ...
        $data['flag'] = 0;
    updateRecord("report", $data, "id = $id");
}

// Delete a specific report
function delete_report($id){
    deleteRecord("report", "id = $id");
}

// Delete all report archived
function delete_archived(){
    $reports = selectQuery("report", "", "id DESC");
    $count = 0;
